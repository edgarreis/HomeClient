...
function connect(){
	...
	console.info('Connecting to Server - CONECTANDO: Hostname: ', hostname, 
	...
	console.info('Connecting... - CONECTANDO');
}
...
function onFail(context) {
	console.log("Failed to connect - FALHA");
}

function onConnectionLost(responseObject) {
	if (responseObject.errorCode !== 0) {
		console.log("Connection Lost - PERDA DE CONECCAO: " + responseObject.errorMessage);
		window.alert("Someone else took my websocket! - ERROR SOCKET\nRefresh to take it back.");
	}
}
...
function led_toggle(){
	...
	client.send(message);
	console.info('sending - ENVIANDO: ', payload);
}
